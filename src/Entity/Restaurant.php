...
class Restaurant
{
    ...
    /**
     * @ORM\ManyToOne(targetEntity=Admin::class, inversedBy="restaurant")
     * @ORM\JoinColumn(nullable=false)
     */
    private $admin;

    /**
     * @ORM\OneToMany(targetEntity=Category::class, mappedBy="restaurant", orphanRemoval=true)
     */
    private $categories;
    ...
    public function addCategory(Category $group): self
    {
        if (!$this->categories->contains($group)) {
            $this->categories[] = $group;
            $group->setRestaurant($this);
        }

        return $this;
    }

    public function removeCategory(Category $group): self
    {
        if ($this->categories->removeElement($group)) {
            // set the owning side to null (unless already changed)
            if ($group->getRestaurant() === $this) {
                $group->setRestaurant(null);
            }
        }
